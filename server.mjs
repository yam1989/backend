...
// ✅ Stable startup (no duplicate defs), listens on PORT
// ✅ Endpoints FIXED (do not change): /, /health, /me, /magic, /magic/status, /magic/result, /video/start, /video/status
// Image Mode:
//   - Default provider: Replicate SDXL img2img (cheap-ish)
//   - Optional provider: OpenAI gpt-image-1 (only if OPENAI_API_KEY is set and provider=openai)
...
const VERSION = "server.mjs DM-2026 STARTS_OK v1.2 (replicate sdxl default + replicate video; openai optional)";
...
// ---------- ENV ----------
const IMAGE_PROVIDER_DEFAULT = (process.env.IMAGE_PROVIDER || "replicate").toLowerCase();
...
// If set, we will send {version,input}; otherwise send {model,input}
const REPLICATE_IMAGE_VERSION = (process.env.REPLICATE_IMAGE_VERSION || "").trim();

// SDXL tuning
const SD_PROMPT_STRENGTH = Number(process.env.SD_PROMPT_STRENGTH || 0.30); // 0.25–0.45 (lower = preserves input more)
const SD_GUIDANCE_SCALE = Number(process.env.SD_GUIDANCE_SCALE || 6.0);
const SD_STEPS = Number(process.env.SD_STEPS || 16);
const SD_NEGATIVE_PROMPT =
  process.env.SD_NEGATIVE_PROMPT ||
  "blurry, low quality, distorted, extra objects, text, watermark, logo, cropped, zoomed, reframed, different composition";

// Frame-lock (optional): make a 1024x1024 square with blurred background to reduce zoom/crop tendencies
const SD_FRAMELOCK_SCALE = Number(process.env.SD_FRAMELOCK_SCALE || 0.78);
...
function getStylePrompt(styleId) {
  const base =
    ...
    "CRITICAL: preserve the SAME composition, shapes, proportions, and positions. " +
    "Do NOT add new objects. Do NOT remove objects. Do NOT change framing or camera. " +
    "Keep it clean, crisp, smooth gradients, gentle shading, premium look. " +
...
async function toPngMax1024(buf) {
  await loadSharp();
  return _sharp(buf).resize({ width:1024, height:1024, fit:"inside", withoutEnlargement:true }).png().toBuffer();
}

async function frameLockSquare1024(pngBuf, scale=0.78) {
  // 1024x1024: blurred cover background + centered contain foreground (smaller) to prevent zoom/crop.
  await loadSharp();
  const fgSize = Math.max(320, Math.round(1024 * Math.min(0.92, Math.max(0.55, scale))));
  const fg = await _sharp(pngBuf).resize({ width: fgSize, height: fgSize, fit:"inside", withoutEnlargement:true }).png().toBuffer();
  const bg = await _sharp(pngBuf).resize({ width:1024, height:1024, fit:"cover" }).blur(18).modulate({ brightness: 1.03, saturation: 1.03 }).png().toBuffer();
  return _sharp(bg).composite([{ input: fg, gravity:"center" }]).png().toBuffer();
}
...
async function bufferToOpenAIFile(buf, filename, mime) {
  await loadOpenAI();
  return _toFile(buf, filename, { type: mime });
}
...
async function runReplicateImageMagic({ jobId, file, styleId }) {
  try {
    ...
    // Prepare image (PNG + optional frame-lock)
    const png = await toPngMax1024(file.buffer);
    const locked = await frameLockSquare1024(png, SD_FRAMELOCK_SCALE);
    const image = bufferToDataUri(locked, "image/png");

    // Validate version if provided
    const ver = REPLICATE_IMAGE_VERSION;
    const useVersion = ver && isHex64(ver);
    ...
    const pred = await replicateCreatePrediction({
      ...
      version: useVersion ? ver : undefined,
      input,
    });
    ...
    while (true) {
      ...
      if (Date.now() - t0 > 180_000) throw new Error("Replicate image timeout");
      await sleep(900);
...
async function runOpenAIImageMagic({ jobId, file, styleId }) {
  try {
    ...
    const png = await toPngMax1024(file.buffer);
    const openaiImage = await bufferToOpenAIFile(png, "input.png", "image/png");
...
app.get("/me", (_req,res)=>res.status(200).json({
  ...
  image: {
    ...
    replicate: { owner: REPLICATE_IMAGE_OWNER, model: REPLICATE_IMAGE_MODEL, version: REPLICATE_IMAGE_VERSION || null, versionPinned: isHex64(REPLICATE_IMAGE_VERSION), prompt_strength: SD_PROMPT_STRENGTH, steps: SD_STEPS, key: Boolean(REPLICATE_API_TOKEN) }
  },
  video: { provider:"replicate", owner: REPLICATE_VIDEO_OWNER, model: REPLICATE_VIDEO_MODEL, version: REPLICATE_VIDEO_VERSION || null, versionPinned: isHex64(REPLICATE_VIDEO_VERSION) },
}));

// POST /magic
app.post("/magic", upload.single("image"), async (req,res)=>{
  try {
    const provider = String(req.body?.provider || IMAGE_PROVIDER_DEFAULT || "replicate").toLowerCase();
    ...
    if (provider === "openai") runOpenAIImageMagic({ jobId:id, file, styleId });
    else runReplicateImageMagic({ jobId:id, file, styleId });
...
app.listen(PORT, "0.0.0.0", () => {
  ...
  console.log(`✅ IMAGE_PROVIDER default: ${IMAGE_PROVIDER_DEFAULT}`);
  console.log(`✅ SDXL: ${REPLICATE_IMAGE_OWNER}/${REPLICATE_IMAGE_MODEL} versionPinned=${isHex64(REPLICATE_IMAGE_VERSION)} version=${REPLICATE_IMAGE_VERSION || "(none)"}`);
});
