...
// GET  /, /health, /me
//
// Added (safe): GET /debug/prediction?id=...  (returns output shape + first URL guess)
...
function outputShape(x) {
  if (x == null) return "null";
  if (typeof x === "string") return "string";
  if (Array.isArray(x)) return `array(len=${x.length})`;
  if (typeof x === "object") return `object(keys=${Object.keys(x).slice(0, 30).join(",")})`;
  return typeof x;
}

// Very tolerant URL extractor for Replicate outputs (images + video)
function normalizeOutputUrl(output) {
  ...
  if (Array.isArray(output)) {
    for (const item of output) {
      const u = normalizeOutputUrl(item);
      if (u) return u;
    }
    return null;
  }
  ...
  if (typeof output === "object") {
    // Common direct keys across models
    const directKeys = [
      "url","uri","file","path",
      "image","png","jpg","jpeg",
      "video","mp4","webm","gif",
    ...
    // Some models return { files: [{ url: ...}] } etc.
    const listKeys = ["files","images","videos","results","outputs"];
    ...
    // Sometimes: { data: {...} } or { result: {...} }
    for (const k of ["data","result"]) {
      if (output[k]) {
        const u = normalizeOutputUrl(output[k]);
        if (u) return u;
      }
    }
  }
...
// Debug helper: inspect a prediction output shape + url guess (does not break existing endpoints)
app.get("/debug/prediction", async (req, res) => {
  try {
    if (!okEnv(res)) return;
    const id = (req.query?.id || "").toString().trim();
    if (!id) return res.status(400).json({ ok: false, error: "Missing id" });
    const p = await replicateGetPrediction(id);
    const url = normalizeOutputUrl(p?.output);
    res.status(200).json({
      ok: true,
      status: p?.status || "unknown",
      outputShape: outputShape(p?.output),
      outputKeys: (p?.output && typeof p.output === "object" && !Array.isArray(p.output)) ? Object.keys(p.output).slice(0, 40) : null,
      urlGuess: url,
      error: p?.error || null,
    });
  } catch (e) {
...
app.post("/magic", upload.single("image"), async (req, res) => {
  try {
    ...
    const basePrompt =
      ...
      "Clean crisp lines, smooth solid color fills, gentle soft shading, no paper texture, no noise. " +
      "Do NOT add new objects, do NOT remove objects, no zoom, no crop.";
    const negative =
      "photorealistic, photo, paper texture, scan artifacts, watermark, text, letters, numbers, border, frame, extra objects, blurry, low quality";

    const input = {
      [IMG_PROMPT_KEY]: `${basePrompt} Style hint: ${styleId}.`.trim(),
...
app.get("/magic/status", async (req, res) => {
  try {
    ...
    res.status(200).json({ ok: true, status, outputUrl, error: p?.error || null, debug: { outputShape: outputShape(p?.output) } });
  } catch (e) {
    res.status(500).json({ ok: false, error: String(e?.message || e) });
  }
});
...
app.post("/video/start", upload.single("image"), async (req, res) => {
  try {
    ...
    const p = await replicateCreatePrediction({ owner: REPLICATE_VIDEO_OWNER, model: REPLICATE_VIDEO_MODEL, version: REPLICATE_VIDEO_VERSION || undefined, input });
    res.status(200).json({ ok: true, id: p.id });
  } catch (e) {
    res.status(500).json({ ok: false, error: String(e?.message || e) });
  }
...
app.listen(PORT, "0.0.0.0", () => console.log(`✅ DM-2026 backend listening on :${PORT}`));
